...
Class longPostToShort{
	public $message, $per_counts;

	public function __construct($message, $per_counts = 500){
		$this->message = $message;
		$this->per_counts = $per_counts;
	}

    	//截取字符串到数组, 默认标签无嵌套
    	public function cutMessage($message, $per_counts = 100,$start = 0){
    	...
    	//返回逆序获取到的第一个特殊字符
    	public function checkSpecialchars($message){
    	...
    	public function checkSpecialCutted($message){
    		$specialchars = array('[font]',
    					'[attach]',
    					'[attachimg]',
    					);
